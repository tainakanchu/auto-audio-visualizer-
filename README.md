...
8. 操作卓: `Q` seed ガチャ、`W` 細部 reroll、`B` 背景、`H` hue 固定トグル、`[` / `]` hue ∓15°、`T` タップテンポ、`,` / `.` テンポ ÷2 / ×2、`/` テンポ AUTO、`L` Timeline lock 30s トグル、`Shift+A` メイン窓オートサイクル。`Q` は Deck 側で具体的な seed を送るのでプロジェクターと `?mirror=1` が同じ画になる。`W` も seed を載せる。
...
| `,` / `.` | テンポ ÷2 / ×2 |
| `/` | テンポ AUTO |
| `Q` | seed ガチャ（メイン窓 `R` 相当。Deck の `R` は bank 再生成。具体的な seed を送る） |
| `W` | 細部 reroll（メイン窓 `D` 相当。seed を載せる） |
| `B` | 背景トグル |
